...
- `pymodbus` runtime constraint (both `manifest.json` and `pyproject.toml [project].dependencies`) is `>=3.11.2,<4` — permissive to cover every pymodbus 3.x version HA-Core has shipped (HA 2026.5/6 ship 3.11.x; HA 2026.7 shipped `pymodbus==3.13.1`, which is why the previous tight `<3.12` pin broke the integration on 2026.7 — see [#88](https://github.com/tomwellnitz/Webasto-Next-Modbus/issues/88)). The **dev group** in `pyproject.toml` still pins `pymodbus<3.12` because `virtual_wallbox/server.py` targets the 3.11 datastore API (`ModbusDeviceContext.store`/`decode`, dropped in 3.12+ in favour of `SimData`/`SimDevice`). Follow-up: port `virtual_wallbox` to the new API, then drop the dev pin so tests run against the same pymodbus as production.
