...
- **REST-controlled entities revert after a few seconds**: setting **LED brightness** or toggling **Free charging** now forces an immediate REST re-fetch and keeps the optimistic value until the wallbox confirms it. Previously the value bounced back to the last cached one on the next Modbus poll, because the regular REST refresh is throttled to 60 s and the optimistic value was dropped too early.
